...
{
    $res = json_decode($service->betrecord($username, $roundDate,5,$startTimeSearch,$endTimeSearch,$page0,$pageLimit,$v), TRUE);
            ...
            if ($PageCount > 1)
                {
                    for ($i =2 ;$i<=$PageCount;$i++)
                        {
                            $res = json_decode($service->betrecord($username, $roundDate,5,$startTimeSearch,$endTimeSearch,$i,$pageLimit,$v), TRUE);
                            $data = $res['Data'];
                            foreach ($data as $value)
                            {
                                if (!\App\Models\GameRecord::where('api_type', $api->id)->where('billNo', $value['WagersID'])->first())
