...
$username = '';
    ...
    $res = json_decode($service->betrecord($username, $roundDate,1,$startTimeSearch,$endTimeSearch,$page0,$pageLimit,$subGameKind =1,$gameType = ''), TRUE);
    ...
    if ($res['Code'] ==0)
    {
        ...
        if (count($data) > 0)
        {
            ...
            {
                if (!\App\Models\GameRecord::where('api_type', $api->id)->where('billNo', $value['WagersID'])->first())
                {
                    $l = strlen($api->prefix);
            ...
            if ($PageCount > 1)
            {
                ...
                {
                    $res = json_decode($service->betrecord($username, $roundDate,1,$startTimeSearch,$endTimeSearch,$i,$pageLimit,$subGameKind =1,$gameType = ''), TRUE);
                    $data = $res['Data'];
                    foreach ($data as $value)
                    {
                        if (!\App\Models\GameRecord::where('api_type', $api->id)->where('billNo', $value['WagersID'])->first())
