...
if(!empty($data['data'])){
    ...
    foreach($data as $k=>$v){
        ...
        $r_mod = \App\Models\GameRecord::where('billNo', $v['WagersID'])->where('api_type', $api_mod->id)->first();
        ...
        if($r_mod){
            ...
            {
                $gameCode = $bbin_ball_list[$gameType];
                $gameType = 5;
            } elseif (in_array($gameType, array_keys($bbin_live_list)))//真人
            {
                $gameCode = $bbin_live_list[$gameType];
                $gameType = 1;
            } elseif (in_array($gameType, array_keys($bbin_lottery_list))) { //彩票
                $gameCode = $bbin_lottery_list[$gameType];
                $gameType = 4;
            } else {
                $gameType = 3;
            }
